...
>>>>>>> b45d9c4de9bbe0525709c1fd851c2760ecfcf365
...
>>>>>>> b45d9c4de9bbe0525709c1fd851c2760ecfcf365
        ...
        public void LoadStructurePage(StructureAdmin structureAdmin)
        {
            ...
            StructureAdminAddUI();
            InitUCStructureAdmin();
            InitLUCStructureAdmin();
            InitActionsStructureAdmin();
        }

        private void StructureAdminAddUI()
        {
            this.structureAdmin.mainGrid.ColumnDefinitions.Add(new ColumnDefinition());
            int oldColumn = Grid.GetColumn(this.structureAdmin.UCstructureList);
            newListControl = new ListStructureUserControl();
            newListControl.Name = "UCStructureListDB";
            Grid.SetColumn(newListControl, oldColumn + 1);
            int oldRowSpan = Grid.GetRowSpan(this.structureAdmin.UCstructureList);
            Grid.SetRowSpan(newListControl, oldRowSpan);
            this.structureAdmin.mainGrid.Children.Add(newListControl);

            this.structureAdmin.Delete.Content = "Back";
            this.structureAdmin.Ok.Content = "Validate";

            newListControl.itemList.ContextMenu.Items.Remove(newListControl.CMRemove);
            this.structureAdmin.UCstructureList.itemList.ContextMenu.Items.Remove(this.structureAdmin.UCstructureList.CMRemove);

            newListControl.CMDuplicate.Header = "Remove Item";
            newListControl.CMDuplicate.Click += DuplicateStructureContextMenu_Click;
            this.structureAdmin.UCstructureList.CMDuplicate.Header = "Add Item";
            this.structureAdmin.UCstructureList.CMDuplicate.Click += DuplicateStructureContextMenu_Click;
        }
        ...
        private void InitUCStructureAdmin()
        {
            currentStructure = new Structure();
            this.structureAdmin.ucStructure.Structure = currentStructure;
        }

        private void InitLUCStructureAdmin()
        {
            zooManager.GetStructures(currentZoo);
        }

        #region GestionStructureInitialisationSubPage
        ...
        private void InitActionsStructureAdmin()
        {
            ...
            this.structureAdmin.UCstructureList.itemList.SelectionChanged += structureAdmin_ListSelectionChanged;
        ...
        private void InitActionsScheduleAdmin()
        {
            //TODO Faire Les scheduleAdmin.
        }
        ...
        private void DuplicateStructureContextMenu_Click(object sender, RoutedEventArgs e)
        {
            if (allStructuresInsert.Contains(this.currentStructure))
            {
                this.structureAdmin.UCstructureList.Obs.Remove(this.currentStructure);
                this.newListControl.Obs.Add(this.currentStructure);
            }
            else if (this.currentZoo.Structures.Contains(currentStructure))
            {
                this.structureAdmin.UCstructureList.Obs.Add(this.currentStructure);
                this.newListControl.Obs.Remove(this.currentStructure);
            }
        }
        private async void btnDelStructure_Click(object sender, RoutedEventArgs e)
        {
            //Back
        }
        private async void btnValidateStructure_Click(object sender, RoutedEventArgs e)
        {
            this.currentZoo.Structures.Clear();
            foreach (var item in this.newListControl.Obs)
            {
                this.currentZoo.Structures.Add(item);
            }
            this.structureAdmin.NavigationService.GoBack();
        }
